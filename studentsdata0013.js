...
class SchoolManagementSystem {
 ...
 setupURLDATA() {
  ...
  if (this.currentSchoolData && this.currentSchoolData.urls) { 
    ...
    // Create URLs with multiple key formats to support different selector values
    const urls = {
      // Standard format
      ...
      // Alternative formats that might be used in selectors
      'FORM ONE': this.currentSchoolData.urls.formOne,
      'FORM TWO': this.currentSchoolData.urls.formTwo,
      'FORM THREE': this.currentSchoolData.urls.formThree,
      'FORM FOUR': this.currentSchoolData.urls.formFour,
      ...
      // Roman numerals (common in school systems)
      'FORM I': this.currentSchoolData.urls.formOne,
      'FORM II': this.currentSchoolData.urls.formTwo,
      'FORM III': this.currentSchoolData.urls.formThree,
      'FORM IV': this.currentSchoolData.urls.formFour,
      
      // Simple forms
      'formOne': this.currentSchoolData.urls.formOne,
      'formTwo': this.currentSchoolData.urls.formTwo,
      'formThree': this.currentSchoolData.urls.formThree,
      'formFour': this.currentSchoolData.urls.formFour
    };
  ...
  // Check if window.nmgFurls exists and has the selected class
  if (typeof window.nmgFurls !== 'undefined' && window.nmgFurls) {
    
    // Try direct match first
    if (window.nmgFurls[this.selectedClass]) {
    ...
    const lowerSelected = this.selectedClass.toLowerCase();
    for (const [key, value] of Object.entries(window.nmgFurls)) {
      if (key.toLowerCase() === lowerSelected) {
        this.nmgFURL = value;
        this.log('✅ Selected Class (case-insensitive match):', this.selectedClass);
        return;
      }
    }
    
    // Try partial matches (for cases like "formOne" matching "FORM-ONE")
    for (const [key, value] of Object.entries(window.nmgFurls)) {
      const cleanKey = key.replace(/[-\s]/g, '').toLowerCase();
      const cleanSelected = this.selectedClass.replace(/[-\s]/g, '').toLowerCase();
      
      if (cleanKey === cleanSelected || 
          cleanKey.includes(cleanSelected) || 
          cleanSelected.includes(cleanKey)) {
        this.nmgFURL = value;
        this.log('✅ Selected Class (partial match):', this.selectedClass, '→', key);
        return;
      }
    }
    
    // If no match found, log available options for debugging
    this.log('❌ Available URL keys:', Object.keys(window.nmgFurls));
    this.log('❌ Attempted to match:', this.selectedClass);
    console.warn('Cannot access URL for class:', this.selectedClass);
    this.showErrorMessage('Invalid class selection. Please check class selector options.');
    
  } else {
    this.log('❌ window.nmgFurls is not defined or empty');
    console.warn('window.nmgFurls not available. Current state:', window.nmgFurls);
    this.showErrorMessage('Class URLs not loaded. Please refresh the page.');
  }
}
