...
function buildSubjectTable(students) {
  ...
  students.forEach(student => {
    ...
    Object.entries(student.scores).forEach(([subject, s]) => {
      ...
      if (subjectStats[subject][s.grade][gender] !== undefined) {
        subjectStats[subject][s.grade][gender]++;
      }

      if (!subjectCounts[subject]) subjectCounts[subject] = { F: 0, M: 0 };
      if (gender === 'F' || gender === 'M') subjectCounts[subject][gender]++;
    });
  });
  ...
  thSubject.rowSpan = 2;
  headerRow1.appendChild(thSubject);

  // SAT-F, SAT-M, TOTAL
  ['SAT-F', 'SAT-M', 'TOTAL'].forEach(text => {
    const th = document.createElement('th');
    th.textContent = text;
    th.rowSpan = 2;
  ...
  // Grades A-F with F/M subcolumns
  ...
  Object.keys(subjectStats).forEach(subject => {
    ...
    // Dummy AVER, GPA, GRADE (you can calculate your own)
    ['-', '-', '-'].forEach(text => {
      const td = document.createElement('td');
      td.textContent = text;
      row.appendChild(td);
  ...
  style.textContent = `
    ...
    table.subject-table th:first-child, table.subject-table td:first-child {
      text-align: left;
      width: 15%;
      font-weight: bold;
    }
    ...
    /* Grades columns */
    table.subject-table th:nth-child(n+5):nth-child(-n+16),
    table.subject-table td:nth-child(n+5):nth-child(-n+16) {
      width: 3.5%;
    }
    /* AVER, GPA */
    table.subject-table th:nth-last-child(3),
    table.subject-table td:nth-last-child(3),
    table.subject-table th:nth-last-child(2),
    table.subject-table td:nth-last-child(2) {
      width: 5%;
    }
    /* GRADE */
    table.subject-table th:nth-last-child(1),
    table.subject-table td:nth-last-child(1) {
      width: 20%;
      text-align: left;
    }
  `;
