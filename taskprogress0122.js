...
function buildSubjectTable(students) {
  ...
  const totalDivF = divisionCount['I'].F + divisionCount['II'].F + divisionCount['III'].F + divisionCount['IV'].F + divisionCount['O'].F;
  const totalDivM = divisionCount['I'].M + divisionCount['II'].M + divisionCount['III'].M + divisionCount['IV'].M + divisionCount['O'].M;
  
  overallStats.INC.F = overallStats.clean.F - totalDivF;
  overallStats.INC.M = overallStats.clean.M - totalDivM;
  
  // ========== BUILD CENTER SUMMARY TABLE ==========
  const centerSummaryTable = document.createElement('table');
  centerSummaryTable.className = 'center-summary-table';

  // Calculate passed candidates (Division I-IV)
  ...
  // Calculate total candidates (Division I-0)
  ...
  // Calculate School GPA (same logic from buildDivisionTable)
  const gradePoints = { A: 1.0, B: 2.0, C: 3.0, D: 4.0, F: 5.0 };
  const divisionPoints = { 'I': 1.0, 'II': 2.0, 'III': 3.0, 'IV': 4.0, 'O': 5.0 };
  ...
  const schoolGPA = (averageSubjectGPA > 0 && averageDivisionGPA > 0)
    ? ((averageSubjectGPA + averageDivisionGPA) / 2).toFixed(4)
    : 'N/A';

  // Build Center Summary Table Header
  ...
  // Build Center Summary Table Body
  const centerSummaryTbody = document.createElement('tbody');

  // Row 1: Total Passed Candidates
  ...
  const gpaRow = document.createElement('tr');
  gpaRow.innerHTML = `
    <td style="font-weight: bold;">SCHOOL GPA</td>
    <td style="text-align: center; font-weight: bold;">${schoolGPA}</td>
  `;
  centerSummaryTbody.appendChild(gpaRow);

  centerSummaryTable.appendChild(centerSummaryTbody);
  
  // Build Overall Analysis Table Header
  ...
  // Build Overall Analysis Table Body
  ...
  subjects.forEach(subject => {
    ...
    // Grade Cells A–F for F, M, and T (Total)
    ['A', 'B', 'C', 'D', 'F'].forEach(grade => {
    ...
    // GRADE Comment with background color
