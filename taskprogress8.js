...
function switchLanguage(lang) {
  ...
  swaBtn.addEventListener("click", () => {
   ...
   const LANG = {
    en: {
      governmentHeader: "Ministry of Education",
      localGov: "Local Government Authority",
      ...
      headComment: "Headmaster’s Comment",
      signature: "Signature",
      date: "Date",
    ...
    sw: {
      governmentHeader: "Wizara ya Elimu",
      localGov: "Mamlaka ya Serikali za Mitaa",
      ...
      headComment: "Maoni ya Mwalimu Mkuu",
      signature: "Sahihi",
      date: "Tarehe",
  ...
  function updateReport(index) {
    ...
    const positionInfo = eligibleStudent ? `<strong>${eligibleStudent.position} out of ${eligibleStudent.total}</strong>` : '';
    ...
    const reportDiv = document.getElementById("report"); // make sure this exists
    ...
    reportDiv.innerHTML = `
      ...
      <div style="display: flex; gap: 20px; margin-top: 5px; flex-wrap: wrap;">
        ...
        <div style="flex: 1; width: 300px;">
          ...
          <table class="behavior-table" style="width: 100%; border-collapse: collapse; margin-top: -20px;">
            <thead>
              <tr><th style="border: 1px solid #ccc;">Aspect</th><th style="border: 1px solid #ccc;">Rating</th></tr>
            </thead>
            <tbody>
              ${Object.entries({
              ...
              <tr><td colspan="2" style="border: 1px solid #ccc;"><strong>Remarks:</strong> ${remark}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div style="margin-top: 10px; font-size: 14px; line-height: 1.5;">
        ...
        <p><strong>${lang.classComment}:</strong> <i>${student.name.toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}</i> 
        has shown ${
        ...
        <br><strong>${lang.signature}:</strong> ${classTeacher} &nbsp; | &nbsp; <strong>${lang.date}:</strong> ${closingDate}</p>
  
        <p><strong>${lang.headComment}:</strong> ${
          student.point <= 13 ? lang.remarks.outstanding :
          student.point <= 17 ? lang.remarks.commendable :
          student.point <= 24 ? lang.remarks.satisfactory : lang.remarks.followup
        }. Continued parental support is recommended.
        <br><strong>${lang.signature}:</strong> ${headmaster} &nbsp; | &nbsp; <strong>${lang.date}:</strong> ${closingDate}</p>
      </div>
    `;
