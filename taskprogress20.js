...
function buildSubjectTable(students) {
  ...
  Object.entries(subjectStats).forEach(([subject, grades]) => {
    ...
    Object.entries(grades).forEach(([grade, sexes]) => {
      const gradeValue = gradeValues[grade];
      const count = sexes.F + sexes.M;
      totalPoints += gradeValue * count;
      totalCount += count;
    });

    const gpa = totalCount > 0 ? (totalPoints / totalCount).toFixed(1) : 'N/A';
    subjectGPAs[subject] = gpa;
  });
