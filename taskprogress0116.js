...
function buildSubjectTable(students) {
  ...
  students.forEach(student => {
    ...
    if (scores.some(sc => sc.mark == null || sc.mark === '')) {
      overallStats.incomplete[gender]++;
    }
  ...
  const overallHeaders = ['CATEGORY', 'REGISTERED', 'SAT', 'CLEAN', 'ABSENT', 'INCOMPLETE', 
  ...
  categorySpacer.style.display = 'none';
  ...
  ['registered', 'sat', 'clean', 'absent', 'incomplete'].forEach(key => {
  ...
  // Add 4 empty <th> cells to match AVER, GPA, POS, GRADE col alignment
  for (let i = 0; i < 4; i++) {
