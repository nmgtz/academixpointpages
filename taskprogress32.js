...
function buildDivisionTable(students) {
  ...
  students.forEach(student => {
    Object.values(student.scores).forEach(score => {
      if (score && typeof score.grade === 'string' && gradeValues[score.grade] !== undefined) {
        totalGPA += gradeValues[score.grade];
        gpaCount++;
      }
    });
  });

  const calcSchoolGPA = gpaCount > 0 ? totalGPA / gpaCount : null;
  ...
  // Append the final table structure
