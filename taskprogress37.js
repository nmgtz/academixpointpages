...
function buildStudentTable(students) {
  ...
  students.forEach((student, index) => {
    student._originalIndex = index;

    const marks = Object.values(student.scores)
      .map(s => parseFloat(s.mark))
      .filter(mark => !isNaN(mark))
      .sort((a, b) => b - a);

    student.best7Total = marks.slice(0, 7).reduce((sum, mark) => sum + mark, 0);
  });

  
  students.sort((a, b) => {
    if (a.point !== b.point) return a.point - b.point;
    if (b.best7Total !== a.best7Total) return b.best7Total - a.best7Total;
    return a._originalIndex - b._originalIndex;
  });

  // Assign position after sorting
