...
buttonAllSub.forEach(button => {
  ...
  const subjectAll = [
     { value: "", innerCont: "Select Subject" },
    { value: "Kiswahili", innerCont: "Kiswahili" },
    { value: "Geography", innerCont: "Geography" },
    { value: "English", innerCont: "English" },
    { value: "History", innerCont: "History" },
    { value: "Biology", innerCont: "Biology" },
    { value: "Civics", innerCont: "Civics" },
    { value: "Civics", innerCont: "Historia ya Tanzania na Maadili" },
    { value: "Commerce", innerCont: "Business Studies" },
    { value: "Mathematics", innerCont: "Mathematics" },
    { value: "Chemistry", innerCont: "Chemistry" },
    { value: "Physics", innerCont: "Physics" },
    { value: "Commerce", innerCont: "Commerce" },
    { value: "Book-keeping", innerCont: "Book-keeping" },
     { value: "Book-keeping", innerCont: "Bible Knowledge" },
    { value: "Edk", innerCont: "E/D ya Kiislam" },
    { value: "Literature", innerCont: "Literature" },
    { value: "Literature", innerCont: "Computer Studies -ICT" },
    { value: "Literature", innerCont: "Agriculture" }
  ];

  subjectAll.forEach(subject => {
    let optSub = document.createElement("option");
    optSub.value = subject.value;
    optSub.textContent = subject.innerCont;
    subjectForClasses.appendChild(optSub);
  });
    
 let examinationTypeAll = document.getElementById("examType");
    let examAlltype = [
        { value: "", innerCont: "Select Examination Type" },
        { value: "Weekly Test", innerCont: "Weekly Test" },
        { value: "Monthly Test", innerCont: "Monthly Test" },
        { value: "Mid term", innerCont: "Mid-Term Exam" },
        { value: "Terminal", innerCont: "Terminal Exam" },
        { value: "Mid term2", innerCont: "Mid-Term Exam 2" },
        { value: "Annual", innerCont: "Annual Exam" },
        { value: "Pre Morck", innerCont: "Pre-Mock Exam" },
        { value: "Morck", innerCont: "Mock Exam" },
        { value: "Pre National", innerCont: "Pre-National Exam" },
...
document.getElementById("submitAllMarks").onclick = function() {
  ...
  .then(response => {
    ...
    return response.text().then(responseText => {
        console.log('Server Response:', responseText); // Logs the server's response

        if (responseText.includes('Invalid subject')) {
        ...
        displayErrorMessage(errorMsg);
            throw new Error('Invalid subject detected'); // This stops execution here
        }

        return responseText; // Continue if no error
    });
})
.then(responseText => {
    console.log('Valid Server Response:', responseText);
    showSuccessMessage("All marked students' marks have been submitted successfully!");
})
.catch(error => {
    if (error.message === 'Invalid subject detected') {
        console.warn('Handled error:', error.message); // Avoid displaying general error
        return; // Exit early, as we already handled this case
    }
