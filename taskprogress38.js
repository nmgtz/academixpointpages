...
function buildStudentTable(students) {
  ...
  // 1. Create a sorted copy of students by point (ascending)
  const sortedByPoint = [...students].sort((a, b) => a.point - b.point);

  // 2. Assign ranking positions by point to students based on sorted array
  // Use a map from student reference to position for quick lookup
  const positionMap = new Map();
  sortedByPoint.forEach((student, index) => {
    positionMap.set(student, index + 1);
  });

  // 3. Build the table with students in original order,
  // but show the position from the map
  ...
  students.forEach((student, i) => {
    ...
    const pos = includePosition ? positionMap.get(student) : '';
    ...
    const cells = [
      ...
      includePosition ? `<td class="pos-column">${pos}</td>` : '',
