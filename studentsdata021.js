...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (addStudentsBtn) {
    addStudentsBtn.addEventListener('click', showStudentsDiv);
  }
  
  if (addSubjectsBtn) {
    addSubjectsBtn.addEventListener('click', showSubjectDiv);
  }
  
  if (showDataBtn) {
    showDataBtn.addEventListener('click', showDataSent);
  }
});
...
updateSelectedList();
    ...
    <div class="table-wrapper">
      <table class="responsive-table">
        ...
        <tbody>
          ...
          <button class="back-btn" id="backEditBtn">Back & Edit Subjects</button>
          <button class="submit-btn" id="submitTableBtn">Submit Students Subjects</button>
          <button class="submit-btn" id="getStudentsBtn">Get Students Names per subjects</button>
  ...
  if (submitTableBtn) {
    submitTableBtn.addEventListener('click', submitTableData);
  }

  const getStudentsBtn = document.getElementById('getStudentsBtn');
  if (getStudentsBtn) {
    getStudentsBtn.addEventListener('click', getDataAndBuildUI);
  }
...
async function getDataAndBuildUI() {
  ...
  try {
    ...
    if (status === 'success') {
      ...
      select.addEventListener('change', handleSubjectSelection);
...
butn.addEventListener('click', () => window.location.reload());
...
backBtn.addEventListener('click', () => displayButton(false));
...
// Add event listener for subject submission button
document.addEventListener('DOMContentLoaded', () => {
  // Wait for the DOM to be fully loaded before setting up the event listener
  setTimeout(() => {
    const subjectSubmitBtn = document.querySelector('.mySubject button');
    if (subjectSubmitBtn) {
      subjectSubmitBtn.addEventListener('click', submitSubjects);
    }
  }, 500);
});
