...
(function() {
  ...
  function createModal() {
    ...
    modal.id = 'logoUploadModal';
    modal.className = 'logo-modal';
    modal.innerHTML = `
      <div class="logo-modal-content">
        <div class="logo-modal-header">
          <h2 class="logo-modal-title">Upload School Logo</h2>
          <button class="logo-close-btn" onclick="closeLogoModal()">&times;</button>
        </div>
        ...
        <div class="logo-modal-actions">
          <button class="logo-btn logo-btn-secondary" onclick="closeLogoModal()">Cancel</button>
          <button class="logo-btn logo-btn-primary" id="logoSaveBtn" disabled>Save Logo</button>
        </div>
      </div>
    `;
  ...
  function saveLogo() {
    ...
    try {
      ...
      displayLogoPreview(croppedImage);
      closeLogoModal();
      alert('Logo saved successfully!');
    } catch (e) {
      alert('Failed to save logo. Image might be too large.');
  ...
  function loadSavedLogo() {
    try {
      ...
      console.error('Failed to load saved logo:', e);
    }
  }
  ...
  function init() {
    ...
    // Find the first form-row and add logo button after it
    const firstFormRow = document.querySelector('.form-row');
    if (firstFormRow) {
      const logoContainer = document.createElement('div');
      logoContainer.style.marginTop = '1em';
      logoContainer.innerHTML = `
        <button type="button" class="logo-upload-btn" onclick="openLogoModal()">
          📷 Upload School Logo
        </button>
