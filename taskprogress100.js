 const NMG_SCRIPT_ = 'https://script.google.com/macros/s/AKfycbxaMpVdGPtkJB94ADOer_FFNrVMIEuxLh4P-knZhBSx5YysMKg2tESUaPR5nhhExWsW/exec';
    ...
    // Initialize school data system first
    ...
    log('🔧 Setting up URL data...');
    
    if (currentSchoolData && currentSchoolData.urls) { 
        log('✅ Server data found, overriding RepResURLs with server data');
        ...
        log('🔄 RepResURLs updated with server data:', RepResURLs);
        
    } else {
        log('⚠️ No server data found, keeping existing RepResURLs from page');
        
        
        if (typeof RepResURLs !== 'undefined') {
            log('✅ Using existing RepResURLs from page:', RepResURLs);
        } else {
            log('❌ No RepResURLs found on page either');
            RepResURLs = null;
        }
    }
    
    log('🔧 URL setup complete. RepResURLs is ready for use.');
    return RepResURLs; 
}
