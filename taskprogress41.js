...
function buildSubjectTable(students) {
  ...
  subjectHeader.rowSpan = 2;
  gradeRow.appendChild(subjectHeader);

  // Grade headers with colspan
  ...
  ['GPA', 'AVER', 'GRADE', 'SAT-F', 'SAT-M', 'TOTAL'].forEach(header => {
    const th = document.createElement('th');
  ...
  subjects.forEach(subject => {
    ...
    gpa.textContent = finalGPAs[subject];
    row.appendChild(gpa);

    const avg = document.createElement('td');
    avg.textContent = finalAverages[subject];
    row.appendChild(avg);

    const gradeComment = document.createElement('td');
    gradeComment.textContent = finalGradesWithComments[subject];
    row.appendChild(gradeComment);

    const satF = document.createElement('td');
    satF.textContent = subjectCounts[subject]?.F || 0;
    row.appendChild(satF);

    const satM = document.createElement('td');
    satM.textContent = subjectCounts[subject]?.M || 0;
    row.appendChild(satM);

    const total = document.createElement('td');
    total.textContent = (subjectCounts[subject]?.F || 0) + (subjectCounts[subject]?.M || 0);
