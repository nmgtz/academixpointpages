...
function buildSubjectTable(students) {
  ...
  // Finalize GPA, Averages, and Grade+Comments
  const finalGPAs = {};
  const finalAverages = {};
  ...
  Object.keys(subjectStats).forEach(subject => {
    ...
    finalGPAs[subject] = g.count > 0 ? (g.totalPoints / g.count).toFixed(1) : 'N/A';
    ...
    if (!isNaN(avgNum)) {
      if (avgNum > 74) gradeComment = 'A (Excellent)';
      else if (avgNum > 64) gradeComment = 'B (Very Good)';
      else if (avgNum > 44) gradeComment = 'C (Good)';
      else if (avgNum > 29) gradeComment = 'D (Satisfactory)';
  ...
  gpaRow.appendChild(document.createElement('td'));
  const gpaLabel = document.createElement('td');
  gpaLabel.textContent = 'SUBJECT GPA';
  gpaLabel.style.fontWeight = 'bold';
  gpaRow.appendChild(gpaLabel);
  subjects.forEach(subject => {
    const td = document.createElement('td');
    td.textContent = finalGPAs[subject];
    td.style.fontWeight = 'bold';
    gpaRow.appendChild(td);
  });
  thead.appendChild(gpaRow);
  ...
  subjects.forEach(subject => {
    const td = document.createElement('td');
    td.textContent = finalAverages[subject];
    td.style.fontWeight = 'bold';
  ...
  gradeRow.appendChild(document.createElement('td'));
  const gradeLabel = document.createElement('td');
  gradeLabel.textContent = 'SUBJECT GRADE';
  gradeLabel.style.fontWeight = 'bold';
  gradeRow.appendChild(gradeLabel);
  subjects.forEach(subject => {
    const td = document.createElement('td');
    td.textContent = finalGradesWithComments[subject];
    td.style.fontWeight = 'bold';
    gradeRow.appendChild(td);
  });
  thead.appendChild(gradeRow);

  // === CAND. rows ===
  ['F', 'M', 'TOTAL'].forEach(type => {
    const row = document.createElement('tr');
    row.appendChild(document.createElement('td'));
    const label = document.createElement('td');
    label.textContent = type === 'TOTAL' ? 'CAND.' : type;
    label.style.fontWeight = 'bold';
    row.appendChild(label);
    subjects.forEach(subject => {
      const female = subjectCounts[subject]?.F || 0;
      const male = subjectCounts[subject]?.M || 0;
      const total = female + male;
      const td = document.createElement('td');
      td.textContent = type === 'F' ? female : type === 'M' ? male : total;
      td.style.fontWeight = 'bold';
      row.appendChild(td);
    });
