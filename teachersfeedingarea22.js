...
function setupNumericInputShortcuts() {
    ...
    if (marksInput) {
        marksInput.addEventListener('keydown', function(event) {
            ...
            // Allow X or x for absent students (keyCode 88 for X, 120 for x)
            if (event.keyCode === 88 || event.keyCode === 120) {
                // Only allow X if input is empty or already contains X
                const currentValue = this.value;
                if (currentValue === '' || currentValue.toUpperCase() === 'X') {
                    // Convert to uppercase X
                    setTimeout(() => {
                        this.value = 'X';
                    }, 0);
                    return;
                } else {
                    // Don't allow X if there are already numbers
                    event.preventDefault();
                }
                return;
            }
            
            // For numbers, don't allow if input already contains X
            if (this.value.toUpperCase() === 'X') {
                event.preventDefault();
                return;
            }
            
            // Ensure that it's a number and stop the keypress for other characters
            if ((event.shiftKey || (event.keyCode < 48 || event.keyCode > 57)) && (event.keyCode < 96 || event.keyCode > 105)) {
                event.preventDefault();
            }
        });
        
        // Handle paste events to ensure only numbers or X are pasted
        marksInput.addEventListener('paste', function(event) {
            ...
            // Check if pasted text is X or x
            if (pastedText.toUpperCase() === 'X') {
                this.value = 'X';
            } else if (/^\d+$/.test(pastedText)) {
                // Only allow if it's purely numeric
