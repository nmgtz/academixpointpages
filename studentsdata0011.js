...
// ========================================
// MAIN UI FUNCTIONS (GLOBAL SCOPE)
// ========================================
...
function showStudentsDiv() {
  (function() {
    ...
    function initializeForm(formType) {
      ...
      if (formRows) formRows.innerHTML = ''; // Clear old rows
      ...
      function addRow(name = '', gender = '') {
        ...
        `;
        if (formRows) formRows.appendChild(row);
      ...
      function handlePaste(input, columnIndex) {
        ...
        input.addEventListener('paste', (event) => {
          ...
          lines.forEach((line, index) => {
            if (formRows && formRows.children[index]) {
...
async function submitSubjects() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (mySubject) {
        mySubject.textContent = 'Data Has been Successful Submited! Thank you for choosing our Product!';
      }
      setTimeout(() => {
        hideLoadingOverlay();
      }, 5000);
...
function handleSubjectSelection() {
  ...
  const subjectIndex = allHeaders.indexOf(selectedValue);
  const nameColumn = allHeaders[0]; // First column assumed to be names

  const filteredNames = allData
    .filter(row => row[selectedValue] === 'V')
    .map(row => row[nameColumn]);

  displayFilteredNames(filteredNames, selectedValue);
}
...
<div class="attendance-wrapper">
  ...
  container.innerHTML = visibleTable;
}

// ========================================
// EVENT LISTENERS INITIALIZATION
// ========================================
// Class selector event listener
if (classSelector) {
  classSelector.addEventListener('change', () => {
    selectedClass = classSelector.value;
    if (typeof nmgFurls !== 'undefined' && nmgFurls[selectedClass]) {
...
// ========================================
// Initialize subjects if DOM elements exist
if (subjectList && selectedList) {
