...
/* ══ BULK UPLOAD STYLES ══ */
'#axpBulkModal{display:none;position:fixed;inset:0;z-index:9998;background:rgba(10,15,40,.72);align-items:flex-start;justify-content:center;padding:18px 12px;overflow-y:auto}'+
'#axpBulkModal.open{display:flex}'+
'#axpBulkBox{background:var(--white);width:100%;max-width:640px;border:1px solid var(--border);animation:popIn .18s ease;margin:auto}'+
'#axpBulkHd{background:var(--pri);padding:14px 18px;display:flex;align-items:center;gap:10px;border-bottom:3px solid var(--gold)}'+
'#axpBulkHd .bhi{font-size:20px;color:var(--gold)}'+
'#axpBulkHd .bht{font-size:13.5px;font-weight:800;color:#fff;flex:1}'+
'#axpBulkHd .bhx{background:none;border:none;color:rgba(255,255,255,.6);font-size:22px;cursor:pointer;line-height:1;padding:0}'+
'#axpBulkHd .bhx:hover{color:#fff}'+
'#axpBulkBody{padding:16px 18px}'+
/* Upload tabs */
'.bulk-tabs{display:flex;gap:0;border-bottom:2px solid var(--border);margin-bottom:16px}'+
'.bulk-tab{padding:8px 16px;font-size:12px;font-weight:700;color:var(--muted);cursor:pointer;border-bottom:3px solid transparent;margin-bottom:-2px;display:flex;align-items:center;gap:6px;transition:color .1s}'+
'.bulk-tab:hover{color:var(--pri)}'+
'.bulk-tab.on{color:var(--pri);border-bottom-color:var(--pri)}'+
'.bulk-tab i{font-size:14px}'+
/* Tab panes */
'.bulk-pane{display:none}.bulk-pane.on{display:block}'+
/* Drop zone */
'.axpDropZone{border:2px dashed var(--border);padding:28px 18px;text-align:center;cursor:pointer;transition:border-color .15s,background .15s;background:var(--panel);position:relative}'+
...
'.axpDropZone i{font-size:32px;color:var(--pri);display:block;margin-bottom:8px}'+
'.axpDropZone .dz-title{font-size:13px;font-weight:700;color:var(--text);margin-bottom:4px}'+
'.axpDropZone .dz-sub{font-size:11px;color:var(--muted)}'+
/* Scan status */
'.axpScanStatus{margin-top:12px;padding:10px 14px;background:var(--panel);border:1px solid var(--border);font-size:12px;display:none}'+
'.axpScanStatus.vis{display:flex;align-items:center;gap:9px}'+
'.scan-ring{width:16px;height:16px;border:2px solid var(--border);border-top-color:var(--pri);border-radius:50%;animation:spin .7s linear infinite;flex-shrink:0}'+
/* Matched table */
'.axpMatchTbl{width:100%;border-collapse:collapse;font-size:11.5px;margin-top:12px;max-height:280px;overflow-y:auto;display:block}'+
'.axpMatchTbl thead{position:sticky;top:0;z-index:1}'+
'.axpMatchTbl th{background:var(--pri);color:#fff;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.7px;padding:7px 9px;text-align:left}'+
'.axpMatchTbl th:not(:nth-child(2)){text-align:center}'+
'.axpMatchTbl td{padding:6px 9px;border-bottom:1px solid var(--border)}'+
'.axpMatchTbl td:not(:nth-child(2)){text-align:center}'+
'.axpMatchTbl tbody tr:nth-child(even){background:var(--panel)}'+
'.match-ok{color:#1a7a4a;font-weight:700}'+
'.match-warn{color:#d46b08;font-weight:700}'+
'.match-err{color:var(--red);font-weight:700}'+
'.match-score-inp{width:55px;padding:3px 6px;font-size:12px;font-weight:700;text-align:center;border:1px solid var(--border);background:var(--panel);color:var(--text);-moz-appearance:textfield}'+
...
'.match-score-inp.changed{border-color:#d46b08;background:#fffbeb}'+
/* CSV textarea */
'.axpCsvArea{width:100%;min-height:140px;padding:10px;font-size:12px;font-family:\'Courier New\',monospace;border:1px solid var(--border);background:var(--panel);color:var(--text);resize:vertical;outline:none}'+
'.axpCsvArea:focus{border-color:var(--pri)}'+
/* Summary bar */
'.axpMatchSummary{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0 6px;font-size:11px;font-weight:700}'+
'.ms-item{padding:4px 10px;border:1px solid var(--border);background:var(--panel)}'+
'.ms-matched{border-color:#1a7a4a;background:#f0fdf4;color:#1a7a4a}'+
'.ms-unmatched{border-color:#d46b08;background:#fff7ed;color:#d46b08}'+
'.ms-nomark{border-color:var(--muted);background:var(--panel);color:var(--muted)}'+
/* Confidence */
'.conf-badge{font-size:9.5px;font-weight:800;padding:2px 6px;letter-spacing:.3px;text-transform:uppercase}'+
'.conf-high{background:#f0fdf4;color:#1a7a4a;border:1px solid #1a7a4a}'+
'.conf-med{background:#fffbeb;color:#8a6800;border:1px solid #d4a017}'+
'.conf-low{background:#fef2f2;color:var(--red);border:1px solid var(--red)}'+
/* Bulk upload trigger button in entry toolbar */
'.axpBulkTrigger{display:inline-flex;align-items:center;gap:6px;padding:7px 13px;font-size:11.5px;font-weight:700;background:#f0fdf4;color:#1a7a4a;border:1px solid #a7f3d0;cursor:pointer;transition:background .1s}'+
'.axpBulkTrigger:hover{background:#d1fae5;border-color:#1a7a4a}'+
    ...
    '<div id="axpBulkBody">'+
      ...
      '<div class="bulk-pane on" id="bpane_scan">'+
        '<div class="axpDropZone" id="axpScanDrop">'+
          ...
          '<div class="dz-title">Drop image or PDF here</div>'+
          '<div class="dz-sub">Supports JPG, PNG, PDF · Screenshot of mark sheet, printed list, or handwritten marks</div>'+
        '</div>'+
...
'<div id="axpToast"></div>';
  ...
  window.axpProcessScanFile=async function(file){
    var status=g('axpScanStatus'),statusTxt=g('axpScanStatusTxt'),preview=g('axpScanPreview'),img=g('axpScanImg'),result=g('axpScanResult');
    result.innerHTML='';g('axpBulkMatchArea').style.display='none';

    /* Show image preview */
    if(file.type.startsWith('image/')){
      var previewUrl=URL.createObjectURL(file);
      img.src=previewUrl; preview.style.display='block';
    }else{
      preview.style.display='none';
    }

    status.classList.add('vis'); statusTxt.textContent='Reading file…';

    try{
      var base64=await fileToBase64(file);
      statusTxt.textContent='Sending to AI for analysis…';

      /* Call Claude API to extract names + scores */
      var prompt=
        'You are extracting student marks from a school mark sheet image or PDF.\n'+
        'The document contains student names and their scores/marks.\n\n'+
        'The students enrolled in this class are:\n'+
        S.students.map(function(s,i){return (i+1)+'. '+s.name;}).join('\n')+'\n\n'+
        'Instructions:\n'+
        '1. Find every student name and their corresponding score/mark in the document.\n'+
        '2. The score must be a number between 0 and 100.\n'+
        '3. Match what you see EXACTLY to the enrolled student list above — do NOT invent names.\n'+
        '4. If a name in the document closely matches an enrolled student, include the enrolled name.\n'+
        '5. Return ONLY a JSON array, no other text:\n'+
        '[{"name":"Exact Name From Document","score":85,"enrolled_match":"Exact Enrolled Name If Different Or Same","confidence":"high|medium|low"},...]';
      ...
      var apiResp=await callClaudeAPI(messages);
      statusTxt.textContent='Matching students…';

      var extracted=parseClaudeJson(apiResp);
      if(!extracted||!extracted.length){
        result.innerHTML='<div style="color:var(--red);font-size:12px;padding:8px 0"><i class="bi bi-exclamation-triangle"></i> No marks could be extracted. Try a clearer image or use the CSV paste method.</div>';
      ...
      status.classList.remove('vis');
      result.innerHTML='<div style="color:var(--red);font-size:12px;padding:8px 0"><i class="bi bi-exclamation-triangle"></i> Scan failed: '+esc(ex.message)+'</div>';
    }
  };
  ...
  function parseCsvText(text){
    ...
    lines.forEach(function(line){
      /* Skip header lines */
      if(/^(name|student|cand|#)/i.test(line))return;
      /* Try formats:
         CandNo, Name, Score  → e.g. S0553-0001, Amina Hassan, 78
         Name, Score          → Amina Hassan, 78
         Name: Score          → Amina Hassan: 78
         Name Score (tab)     → Amina Hassan\t78
      */
      var name='', score=null;

      /* Colon format: Name: Score */
      var colonM=line.match(/^(.+?)\s*:\s*(\d{1,3})\s*$/);
      if(colonM){name=colonM[1].trim();score=parseInt(colonM[2]);if(score>=0&&score<=100){results.push({name:name,score:score,confidence:'high'});return;}}

      /* Split by comma or tab */
      var parts=line.split(/[,\t]/).map(function(p){return p.replace(/^["'\s]+|["'\s]+$/g,'').trim();}).filter(Boolean);
      if(parts.length<2)return;

      /* Find the score — last numeric part between 0-100 */
      var scoreIdx=-1;
      ...
      /* Determine name portion — skip candidate number patterns like S0553-0001 */
      var nameParts=parts.slice(0,scoreIdx).filter(function(p,i){
        /* Skip cand-no pattern */
        return !/^S\d{3,}-\d{4}$/i.test(p);
      });
      if(!nameParts.length)return;
      name=nameParts.join(' ').trim();
      if(name&&score!==null&&score>=0&&score<=100){
        results.push({name:name,score:score,confidence:'high'});
      }
    });
    return results;
  }
  ...
  /*
     ...
     1. Exact match (case-insensitive) → confidence HIGH
     2. All tokens present in any order → confidence HIGH
     3. High Levenshtein similarity (>= 0.80) → confidence MEDIUM
     4. Partial token overlap (>= 60%) → confidence LOW
     5. No match → unmatched

     We NEVER auto-apply LOW confidence matches silently.
     All results are shown in a review table with editable scores.
  */
  function matchExtractedToStudents(extracted){
    var results=[];
    var usedSystemIdx={}; /* prevent double-assigning same student */

    extracted.forEach(function(ext){
      var extName=String(ext.name||'').trim();
      var score=parseInt(ext.score);
      if(isNaN(score)||score<0||score>100){
        results.push({extName:extName,score:null,systemIdx:-1,systemName:'',confidence:'none',status:'invalid_score'});
        return;
      }

      var bestIdx=-1, bestConf='none', bestScore=0;
      ...
      S.students.forEach(function(stu,si){
        ...
        if(sim>bestScore){bestScore=sim;bestIdx=si;bestConf=conf;}
      });

      if(bestIdx>-1&&bestConf!=='none'){
        /* Mark as used only for HIGH/MEDIUM — LOW stays tentative */
        ...
        results.push({extName:extName,score:score,systemIdx:-1,systemName:'',confidence:'none',status:'unmatched'});
      }
    });
    return results;
  }
  ...
  function renderMatchTable(matches){
    ...
    var matched=matches.filter(function(m){return m.status==='matched'&&m.score!==null;}).length;
    var unmatched=matches.filter(function(m){return m.status==='unmatched';}).length;
    var invalid=matches.filter(function(m){return m.status==='invalid_score';}).length;
    var nomark=S.students.length-matched;

    summary.innerHTML=
      '<span class="ms-item ms-matched"><i class="bi bi-check-circle"></i> '+matched+' matched</span>'+
      (unmatched?'<span class="ms-item ms-unmatched"><i class="bi bi-question-circle"></i> '+unmatched+' unmatched</span>':'')+
      (invalid?'<span class="ms-item" style="border-color:var(--red);background:#fef2f2;color:var(--red)"><i class="bi bi-x-circle"></i> '+invalid+' invalid</span>':'')+
      '<span class="ms-item ms-nomark">'+nomark+' students still without mark</span>';

    body.innerHTML=matches.map(function(m,i){
      var confBadge='<span class="conf-badge conf-'+({'high':'high','medium':'med','low':'low','none':'low'}[m.confidence]||'low')+'">'+
        ({'high':'HIGH','medium':'MED','low':'LOW','none':'NO MATCH'}[m.confidence]||'—')+'</span>';

      var systemCell= m.status==='matched' ?
        '<span style="font-weight:600;color:var(--text)">'+esc(m.systemName)+'</span>' :
        '<select class="match-score-inp" style="width:100%;font-family:inherit;font-size:11px" onchange="axpBulkReassign('+i+',this.value)">'+
          '<option value="">— select student —</option>'+
          S.students.map(function(s,si){return'<option value="'+si+'">'+esc(s.name)+'</option>';}).join('')+
        '</select>';

      var scoreCell='<input type="number" class="match-score-inp" value="'+(m.score!==null?m.score:'')+'" min="0" max="100" '+
        'oninput="axpBulkScoreEdit('+i+',this)" placeholder="0-100">';

      var checkCell= m.status==='matched'&&m.score!==null ?
        '<input type="checkbox" '+(m.confidence!=='low'?'checked':'')+' onchange="axpBulkToggle('+i+',this.checked)" style="width:16px;height:16px;cursor:pointer" '+
        'title="'+(m.confidence==='low'?'Low confidence — review carefully before checking':'Include this mark')+'">':
        '<span style="color:var(--muted);font-size:11px">—</span>';

      var rowStyle= m.confidence==='low'?'background:#fffbeb':
  ...
  window.axpBulkToggle=function(idx,checked){
    if(window._axpBulkMatches&&window._axpBulkMatches[idx]){
      window._axpBulkMatches[idx]._selected=checked;
    }
  };
  ...
  window.axpBulkReassign=function(idx,sysIdx){
    ...
    if(isNaN(si)||si<0||si>=S.students.length){
      ...
      window._axpBulkMatches[idx].systemName=S.students[si].name;
      window._axpBulkMatches[idx].status='matched';
      window._axpBulkMatches[idx].confidence='high'; /* Teacher manually confirmed */
    }
  };
  ...
  window.axpApplyBulkMarks=function(){
    ...
    /* Read current checkbox states directly from DOM for accuracy */
    var checkboxes=g('axpMatchBody').querySelectorAll('input[type=checkbox]');
    var toApply=[];
    var skippedLow=0, skippedUnchecked=0;

    matches.forEach(function(m,i){
      if(m.status!=='matched'||m.score===null||m.systemIdx<0)return;
      var cb=checkboxes[i];
      var isChecked=cb?cb.checked:false;
      if(!isChecked){skippedUnchecked++;return;}
      /* Extra safety: validate student index still points to same student */
      if(S.students[m.systemIdx].name!==m.systemName){
        /* Student list changed — skip */
        return;
      }
      /* Validate score range */
      var sc=parseInt(m.score);
      if(isNaN(sc)||sc<0||sc>100)return;
      toApply.push({studentIdx:m.systemIdx,name:m.systemName,score:sc,conf:m.confidence});
    });

    if(!toApply.length){
      axpAlert('Nothing Selected','Check the boxes next to the marks you want to apply.','warning');
      return;
    }

    /* Warn if any low-confidence marks included */
    ...
    var msg='Apply <strong>'+toApply.length+'</strong> mark(s) to the student list?'+
      (lowConf.length?'<br><br><span style="color:#d46b08"><i class="bi bi-exclamation-triangle"></i> '+lowConf.length+' mark(s) have LOW confidence match — please verify.</span>':'')+
      (skippedUnchecked?'<br><span style="font-size:11px;color:var(--muted)">'+skippedUnchecked+' unchecked row(s) will be skipped.</span>':'')+
  ...
  function levenshteinSim(a,b){
    ...
    for(var i2=1;i2<=lb;i2++){
      for(var j2=1;j2<=la;j2++){
        matrix[i2][j2]=b[i2-1]===a[j2-1]?matrix[i2-1][j2-1]:
          Math.min(matrix[i2-1][j2-1]+1,matrix[i2][j2-1]+1,matrix[i2-1][j2]+1);
      }
    }
