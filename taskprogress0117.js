...
function buildSubjectTable(students) {
  ...
  const overallStats = {
    ...
    clean: { F: 0, M: 0 },
    absent: { F: 0, M: 0 },
    inc: { F: 0, M: 0 }  // Changed from 'incomplete' to 'inc'
  };
  ...
  students.forEach(student => {
    ...
    // Incomplete (has some null/empty marks)
    const scores = Object.values(student.scores || {});
    if (scores.length > 0 && scores.some(sc => sc.mark == null || sc.mark === '')) {
      overallStats.inc[gender]++;  // Changed from 'incomplete' to 'inc'
    }
  ...
  // General stats columns - FIXED: Changed 'incomplete' to 'inc'
