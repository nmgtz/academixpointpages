...
(function () {
  ...
  /* ══════════════════════════════════════════════
     WIPE PAGE — remove ALL existing styles + body content
     so Blogger template CSS/HTML does not interfere
  ══════════════════════════════════════════════ */
  function wipePage() {
    /* 1. Remove every <style> and <link rel="stylesheet"> from <head> */
    var toRemove = Array.from(document.querySelectorAll(
      'head style, head link[rel="stylesheet"]'
    ));
    toRemove.forEach(function(el){ el.parentNode.removeChild(el); });

    /* 2. Clear entire <body> content */
    document.body.innerHTML = '';

    /* 3. Reset body base styles so nothing bleeds in */
