...
function renderStudentIDCards(students) {
  ...
  function updateIDCard(startIndex) {
  ...
  // Calculate how many cards to show (max 8 per page)
  const endIndex = Math.min(startIndex + 8, students.length);
  const cardsToShow = students.slice(startIndex, endIndex);
  
  // Build grid of 8 ID cards (4 rows x 2 columns)
  let cardsHTML = '';
  
  cardsToShow.forEach((student, idx) => {
    const actualIndex = startIndex + idx;
    const genderText = student.gender === 'F' ? lang.female : lang.male;
    const studentNumber = currentSchoolData?.indexNumber
      ? `${currentSchoolData.indexNumber.toUpperCase()}-${String(actualIndex + 1).padStart(4, '0')}`
    ...
    cardsHTML += `
      ...
      ">
        ...
        <!-- Header -->
        <div style="text-align: center; margin-bottom: 8px; position: relative; z-index: 2;">
          ${schoolLogo ? `
            ...
            ">
          ` : ''}
          <h3 style="margin: 3px 0; font-size: 9px; text-transform: uppercase; letter-spacing: 0.5px;">
            ${lang.title}
          </h3>
          <p style="margin: 0; font-size: 7px; opacity: 0.9;">
            ${(currentSchoolData?.schoolName && currentSchoolData?.indexNumber)
        ...
        ">
          ...
          <div style="
            ...
            flex-shrink: 0;
          ">
            <i class="bi bi-person-circle" style="font-size: 30px; opacity: 0.7;"></i>
          </div>
          
          <!-- Student Info -->
          <div style="flex: 1; font-size: 7px;">
            <div style="margin-bottom: 4px;">
              <strong style="opacity: 0.8; font-size: 6px;">${lang.name}:</strong><br>
              <span style="font-size: 8px; font-weight: bold; line-height: 1.2;">${student.name.toUpperCase()}</span>
            </div>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; font-size: 6px;">
              <div>
                <strong>${lang.studentNo}:</strong><br>
                <span style="font-size: 7px;">${studentNumber}</span>
              </div>
              <div>
                <strong>${lang.class}:</strong><br>
                <span style="font-size: 7px;">${examClass}</span>
              </div>
              <div>
                <strong>${lang.gender}:</strong><br>
                <span style="font-size: 7px;">${genderText}</span>
              </div>
              <div>
                <strong>${lang.validUntil}:</strong><br>
                <span style="font-size: 7px;">${validYear}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  // Wrapper with 8-card grid layout
  idCardDiv.innerHTML = `
    <div style="
      width: 100%;
      max-width: 1000px;
  ...
  idPrevBtn.disabled = startIndex === 0;
  idNextBtn.disabled = endIndex >= students.length;
}

// Update the navigation to work with pages of 8
currentIndex = 0; // Now represents the starting index of current page
...
idNextBtn.onclick = () => {
  if (currentIndex + 8 < students.length) {
    currentIndex += 8;
    updateIDCard(currentIndex);
  }
};

// Update the download functions to handle the new layout
idDownloadBtn.addEventListener("click", () => {
  ...
  const sanitize = (str) => str.replace(/[^a-z0-9]/gi, '_').replace(/_+/g, '_');
  const pageNumber = Math.floor(currentIndex / 8) + 1;
  const filename = `${sanitize(schoolIndex)}_ID_Cards_Page_${pageNumber}.pdf`;
...
idDownloadAllBtn.addEventListener("click", async () => {
  ...
  try {
    for (let i = 0; i < students.length; i += 8) {
      ...
      idDownloadAllBtn.textContent = `Downloading Page ${pageNum}/${totalPages}`;
      ...
      const filename = `${sanitize(schoolIndex)}_ID_Cards_Page_${pageNum}.pdf`;
    ...
    if (isDownloadingAll) {
      alert(`Download Complete!\nSuccessfully downloaded: ${downloadedCount} pages (${students.length} ID cards)`);
    }
  } catch (error) {
    console.error('Error during batch download:', error);
  } finally {
...
function renderStudentCertificates(students) {
  ...
  // Filter only passing students (Division I-IV)
  const passingStudents = students.filter(s => 
    ['I', 'II', 'III', 'IV'].includes(s.division)
  ).sort((a, b) => a.point - b.point);
  ...
  if (passingStudents.length === 0) {
    reportContainer.innerHTML = `
      <div style="text-align: center; padding: 2em; color: #d32f2f;">
        ...
        <p>No students with passing grades (Division I-IV) found for certificate generation.</p>
      </div>
    `;
    return;
  }
  ...
  const CERT_LANG = {
    en: {
      ...
      recognition: "In recognition of outstanding academic performance",
      achievement: "For achieving",
      division: "Division",
      points: "Total Points",
      examination: "in the",
      examination2: "Examination",
      conducted: "Conducted at",
      date: "Date",
      signature: "Signature",
    ...
    sw: {
      certificate: "CHETI CHA MAFANIKIO",
      presentedTo: "Cheti hiki kinatokewa kwa",
      recognition: "Kutambua mafanikio makubwa ya kitaaluma",
      achievement: "Kwa kufikia",
      division: "Daraja",
      points: "Jumla ya Alama",
      examination: "katika mtihani wa",
      examination2: "",
      conducted: "Uliofanyika",
      date: "Tarehe",
      signature: "Sahihi",
  ...
  function updateCertificate(index) {
    ...
    const student = passingStudents[index];
    const examType = document.getElementById('examType').value.trim();
    const examClass = document.getElementById('classType').value;
    const selectedClass = examClass;
    const savedClassData = SchoolInfoStorage.getClassData(selectedClass);
    ...
    certificateDiv.innerHTML = `
      ...
      ">
        ...
        <!-- School Info -->
        <div style="text-align: center; margin-bottom: 30px;">
          <h2 style="margin: 5px 0; font-size: 24px; color: #333;">
            ${(currentSchoolData?.schoolName && currentSchoolData?.indexNumber)
              ? currentSchoolData.schoolName.toUpperCase()
              : schoolName.toUpperCase()}
          </h2>
        ...
        <div style="text-align: center; margin: 40px 0;">
          <p style="font-size: 18px; font-style: italic; color: #666; margin-bottom: 20px;">
            ${lang.presentedTo}
          </p>
          ...
          <p style="font-size: 16px; color: #666; margin: 20px 0; line-height: 1.8;">
            ${lang.recognition}<br>
            ${lang.achievement}
          </p>
          ...
          ">
            <div style="font-size: 24px; font-weight: bold;">
              ${lang.division} ${student.division}
            </div>
            <div style="font-size: 14px; margin-top: 5px; opacity: 0.9;">
              ${lang.points}: ${student.point}
            </div>
          </div>
          
          <p style="font-size: 16px; color: #666; margin: 20px 0;">
            ${lang.examination} <strong>${examType.toUpperCase()}</strong> ${lang.examination2}<br>
    ...
    certNextBtn.disabled = index === passingStudents.length - 1;
  }
  ...
  certNextBtn.onclick = () => {
    if (currentIndex < passingStudents.length - 1) {
      currentIndex++;
      updateCertificate(currentIndex);
    }
  };
...
// 2. Download Single Certificate Button
certDownloadBtn.addEventListener("click", () => {
  ...
  const student = passingStudents[currentIndex];
  const schoolIndex = currentSchoolData?.indexNumber 
    ? currentSchoolData.indexNumber.toUpperCase() 
    : document.getElementById('schoolIndexInput').value.trim().toUpperCase();
...
// 3. Download All Certificates Button
certDownloadAllBtn.addEventListener("click", async () => {
  ...
  try {
    for (let i = 0; i < passingStudents.length; i++) {
      ...
      certDownloadAllBtn.textContent = `Downloading ${downloadedCount + 1}/${passingStudents.length}`;
...
// =====================================================
// ADD THESE FETCH FUNCTIONS (Insert around line 1400)
// =====================================================
