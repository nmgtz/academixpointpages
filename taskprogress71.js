...
// Function to create and download Excel file with student marks
async function createExcelFile(students, examType = '', selectedClass = '') {
  try {
    // Import XLSX library (make sure to include it in your HTML)
    ...
    students.forEach(student => {
      if (student.subjects) {
        student.subjects.forEach(subject => {
          allSubjects.add(subject.title || subject.name || subject.subject);
        });
      }
    });
    ...
    // Create header row
    const headers = ['S/N', 'Student Name', ...subjectsList];
    
    // Create data rows
    const rows = students.map((student, index) => {
      const row = [
        index + 1, // S/N
        student.name || student.studentName || `Student ${index + 1}` // Student Name
      ];
      
      // Add marks for each subject
      subjectsList.forEach(subjectTitle => {
        const subject = student.subjects?.find(sub => 
          (sub.title || sub.name || sub.subject) === subjectTitle
        );
        row.push(subject?.marks || subject?.score || '-');
      });
    ...
    const colWidths = [
      { wch: 6 },  // S/N
      { wch: 25 }, // Student Name
      ...subjectsList.map(() => ({ wch: 12 })) // Subject columns
    ];
    ws['!cols'] = colWidths;
...
// Enhanced version of your fetchScores function with Excel export
...
// Alternative function to export existing data (if you already have students data)
function exportCurrentData() {
  // Assuming you have students data stored somewhere
  const students = getCurrentStudentsData(); // You'll need to implement this
  const examType = document.getElementById('examType').value.trim();
  const selectedClass = document.getElementById('classType').value.trim();
...
// Helper function to get current students data (implement based on your data structure)
function getCurrentStudentsData() {
  // This should return your current students array
  // You might need to adapt this based on how your data is stored
...
function buildSubjectTable(students) {
  ...
  avgCanvas.id = 'avgMarksChart';
  avgCanvas.height = 300;
  chartContainer.appendChild(avgCanvas);

  const gpaCanvas = document.createElement('canvas');
  gpaCanvas.id = 'gpaChart';
  gpaCanvas.height = 300;
  chartContainer.appendChild(gpaCanvas);

  table.after(chartContainer);

  setTimeout(() => {
    const labels = subjects;
    const averageMarks = labels.map(subject => parseFloat(finalAverages[subject]));
    const gpaValues = labels.map(subject => parseFloat(finalGPAs[subject]));
    ...
    new Chart(avgCanvas, {
      ...
      options: {
        responsive: true,
        plugins: { title: { display: true, text: 'Average Marks per Subject' } },
    ...
    new Chart(gpaCanvas, {
      ...
      options: {
        responsive: true,
        plugins: { title: { display: true, text: 'GPA per Subject' } },
        scales: { y: { beginAtZero: true, max: 5, reverse: true } }
      }
    });
  }, 0);
